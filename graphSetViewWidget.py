...
class GraphSetViewWidget(QWidget):

    def __init__(self, parent = None, generalSetting = {}, graphSetSetting = {}, dataSetSetting = {}, dataSet = [], imageScale = 1):
        super(GraphSetViewWidget, self).__init__(parent)
        ...
        self.createGraphSetView(graphSetSetting, dataSet)
    ...
    def createGraphSetView(self, graphSetSetting, dataSet):
        ...
        for i in range(len(graphSetSetting["graphs"])):
            ...
            pwList.append(pg.PlotWidget(viewBox = pg.ViewBox(border = pg.mkPen(color='#000000', width = self.imageScale),
                                                             invertX = invertXTF, invertY = invertYTF)))

            self.lw.addWidget(pwList[i],
                              graphSetting["position"]["r"], graphSetting["position"]["c"],
                              graphSetting["span"]["r"], graphSetting["span"]["c"])

            # [6.3-4] 
            pwList[i].setBackground(graphSetting["backgroundColor"])

            if graphSetting["size"]["h"] != 0 and graphSetting["size"]["w"] != 0:
                pwList[i].setMinimumSize(graphSetting["size"]["w"], graphSetting["size"]["h"])
                pwList[i].setMaximumSize(graphSetting["size"]["w"], graphSetting["size"]["h"])
            ...
            for j in range(len(graphSetting["plots"])):
                ...
                if plotSetting["style"].startswith("Line"):
                    self.pIList[i][plotSetting["yAxis"]].addItem(
                        pg.PlotCurveItem(dataSet[self.getDataIndex(plotSetting["x"])][dataScopeMin:dataScopeMax],
                                         dataSet[self.getDataIndex(plotSetting["y"])][dataScopeMin:dataScopeMax],
                ...
                if plotSetting["style"].startswith("Point"):
                    self.pIList[i][plotSetting["yAxis"]].addItem(
                        pg.ScatterPlotItem(dataSet[self.getDataIndex(plotSetting["x"])][dataScopeMin:dataScopeMax],
                                           dataSet[self.getDataIndex(plotSetting["y"])][dataScopeMin:dataScopeMax],
